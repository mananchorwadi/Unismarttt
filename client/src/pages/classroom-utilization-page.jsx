...
export default function ClassroomUtilizationPage() {
  ...
  if (!user || user.role !== ROLES.STUDENT) {
    return <DashLayout title="Access Denied" description="This page is only available to students">
  ...
  useEffect(() => {
    ...
    const endDate = new Date(now.getTime() + 60 * 60 * 1e3);
  ...
  >
    ...
    /* Search Results */
        ...
        {freeClassrooms && freeClassrooms.length > 0 && <Card>
            <CardHeader>
            ...
            <CardContent>
              <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                {freeClassrooms.map((classroom) => <Card key={classroom.id} className="relative">
                    <CardContent className="pt-4">
                      <div className="space-y-3">
                        ...
                        {classroom.building && <div className="flex items-center gap-2 text-sm text-muted-foreground">
                            <MapPin className="h-4 w-4" />
                            {classroom.building}
                          </div>}
                        
                        {classroom.capacity && <div className="flex items-center gap-2 text-sm text-muted-foreground">
        ...
        {freeClassrooms && freeClassrooms.length === 0 && selectedDay && startTime && endTime && <Card>
            <CardContent className="pt-6">
        ...
        {allClassrooms && <Card>
            ...
            <CardContent>
              <div className="grid gap-2 md:grid-cols-2 lg:grid-cols-3">
                {allClassrooms.map((classroom) => <div key={classroom.id} className="flex items-center justify-between p-2 rounded-lg border">
                    <div>
                      <span className="font-medium">{classroom.roomNo}</span>
                      {classroom.building && <span className="text-sm text-muted-foreground ml-2">• {classroom.building}</span>}
                    </div>
                    {classroom.capacity && <Badge variant="outline" className="text-xs">
