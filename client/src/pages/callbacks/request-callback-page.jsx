...
export default function RequestCallbackPage() {
  ...
  if (!user || user.role !== "student") {
    return <DashLayout title="Access Denied" description="Only students can access this page">
          ...
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="space-y-2">
                ...
                <Select
    value={formData.facultyId || void 0}
  ...
  >
                  ...
                  <SelectContent>
                    {facultyLoading ? <SelectItem value="loading" disabled>
                        <div className="flex items-center gap-2">
                          <Loader2 className="h-4 w-4 animate-spin" />
                          <span>Loading faculty...</span>
                        </div>
                      </SelectItem> : !facultyList || facultyList.length === 0 ? <SelectItem value="no-faculty" disabled>
                        <span>No faculty members available</span>
                      </SelectItem> : facultyList.map((faculty) => <SelectItem key={faculty.id} value={faculty.id.toString()}>
                          <div className="flex items-center gap-2">
        ...
        <Card>
          ...
          <CardContent>
            {requestsLoading ? <div className="flex items-center justify-center py-8">
              ...
              </div> : !requests || requests.length === 0 ? <div className="text-center py-8 text-muted-foreground">
                <Clock className="h-12 w-12 mx-auto mb-4 opacity-50" />
                <p>No requests yet. Create your first callback request!</p>
              </div> : <div className="space-y-4">
                {requests.map((request) => <div key={request.id} className="border rounded-lg p-4 space-y-3">
