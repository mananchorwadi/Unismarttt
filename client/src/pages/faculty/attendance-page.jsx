...
import { Calendar as CalendarIcon, FileText, UserPlus, BarChart3 } from "lucide-react";
...
export default function AttendancePage() {
  ...
  useEffect(() => {
    ...
    checkServer();
    const interval = setInterval(checkServer, 1e4);
  ...
  const handleStartAttendance = async () => {
    ...
    setAttendanceData([]);
    const lectureData = {
      lecture_id: `${selectedClass}_${formatDateForId(selectedDate)}`,
      // <-- THE FIX IS HERE
      course_code: selectedClass,
      instructor: user?.name || "Prof. Michael Brown",
      // Fetch from profile or use default
      room: "A-317"
      // Default room
    };
...
Total students marked: ${result.attendance_count}`);
  ...
  return <DashLayout title="Smart Attendance" description="AI-powered attendance tracking system">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="md:col-span-2">
          ...
          <CardContent className="space-y-4">
            {!serverConnected ? <div className="bg-red-50 border border-red-200 rounded-lg p-4 text-red-800">
              ...
              </div> : <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 text-blue-800">
                <strong>System Status:</strong> Camera {systemStatus.camera_running ? "\u{1F7E2} Active" : "\u26AA Stopped"} | Registration {systemStatus.registration_running ? "\u{1F7E1} In Progress" : "\u26AA Ready"}
              </div>}
            ...
            {showRegistrationForm && <div className="border rounded-lg p-4 space-y-4">
            ...
            <div className="border rounded-lg">
              ...
              <Table>
                ...
                <TableBody>
                  {attendanceData.length > 0 ? attendanceData.map((student) => <TableRow key={student.id}>
                    ...
                    </TableRow>) : <TableRow><TableCell colSpan={5} className="text-center">No attendance marked yet for this session.</TableCell></TableRow>}
                </TableBody>
              </Table>
            </div>
          </CardContent>
